...
# List of models
models = ["gpt-4o", "gpt-4.1", "gpt-4o-mini", "gpt-4", "gpt-4-turbo", "gpt-3.5-turbo"]
...
def process_xls(file):
    try:
        ...
        st.error(f"Error processing xls: {e}")
        return None, None
...
# File Uploader
uploaded_file = st.file_uploader("Upload a file (CSV, PDF, or Image)", type=["csv", "pdf", "png", "jpg", "jpeg", "xlsx", "xls"])
file_context = ""
...
if uploaded_file:
    ...
    if file_type == "xlsx":
        process_xlsx(uploaded_file)
    if file_type == "xls":
        process_xls(uploaded_file)
    elif file_type == "pdf":
